...
public:
  ...
  ~ScopedVarName() noexcept(false) { mapping_->erase(var_); }
...
static const at::cuda::NVRTC &nvrtc() { return at::globalContext().getNVRTC(); }

std::string CudaPrinterTssa::dtypeToCppString(const Dtype &dtype) {
...
void CudaAnalysisTssa::visit(FreePtr v) {
...
void CudaAnalysisTssa::visit(AllocatePtr v) {
...
void CudaAnalysisTssa::visit(PlacementAllocatePtr v) {
  throw std::runtime_error("Memory reuse not supported yet");
}

void CudaAnalysisTssa::visit(ForPtr v) {
  // Recurse first.
  v->body()->accept(this);
  ...
  if (loop_options.is_gpu_block_index()) {
    ...
    if (!is_zero(v->start())) {
      throw std::runtime_error("start must be zero for gpu_block_index: " +
    ...
    if (!is_zero(v->start())) {
      throw std::runtime_error("start must be zero for gpu_thread_index: " +
                               std::to_string(v->start()));
    }
...
void CudaPrinterTssa::print_flat_alloc(AllocatePtr alloc) {
...
void CudaPrinterTssa::visit(AllocatePtr v) {
...
void CudaPrinterTssa::visit(FreePtr v) {
  // do nothing
}

void CudaPrinterTssa::visit(ForPtr v) { IRPrinter::visit(v); }

void CudaPrinterTssa::visit(CastPtr v) {
  std::string castFn =
      v->dtype().scalar_type() == ScalarType::Half       ? "__float2half"
...
void CudaPrinterTssa::visit(IntrinsicsPtr v) {
...
void CudaPrinterTssa::visit(ExternalCallPtr v) {
  throw unimplemented_lowering(v);
}

void CudaPrinterTssa::visit(LoadPtr v) {
...
public:
  ...
  AtomicAddFuser(const std::unordered_set<VarPtr> &thread_local_bufs,
                 const GPUMetaVarRewriterTssa &metavars)
...
void CudaPrinterTssa::visit(StorePtr v) {
  emitIndent();
  if (v->indices().empty()) {
    os() << *v->base_handle() << " = ";
  } else {
...
void CudaPrinterTssa::visit(AtomicAddPtr v) {
  emitIndent();
...
void CudaPrinterTssa::visit(MaxPtr v) {
...
void CudaPrinterTssa::visit(MinPtr v) {
  if (v->dtype().is_integral()) {
    os() << "min(";
  } else {
    os() << "minimum(";
...
void CudaPrinterTssa::visit(IfThenElsePtr v) {
  os() << "((";
...
void CudaPrinterTssa::visit(BlockPtr v) {
  os() << "{" << std::endl;
  indent_++;
...
void CudaPrinterTssa::visit(LetPtr v) {
  emitIndent();
...
private:
  ...
  void PushList() { load_stack_.push_back(MemLoadList()); }

  void PopList() { load_stack_.pop_back(); }
...
std::string CudaCodeGenTssa::GetUniqueFuncName(const std::string &func_prefix) {
  int64_t counter = 0;
...
bool GPUMetaVarRewriterTssa::isFullExtent() {
  {
...
StmtPtr GPUMetaVarRewriterTssa::mutate(ForPtr v) {
  StmtPtr body = v->body();
  ...
  if (loop_options.is_gpu_block_index()) {
    ...
    // std::cout << to_string(metaVar) << std::endl;
    ...
    if (old_reach->isConstant() && immediateEquals(old_reach, 1)) {
      auto result = v->stop();
      current_thread_reach_[gpu_thread_index] = result;
      // std::cout << to_string(result) << std::endl;
    } else {
      auto result =
          IRSimplifier::simplify(alloc<Max>(old_reach, v->stop(), true));
...
StmtPtr GPUMetaVarRewriterTssa::mutate(BlockPtr v) {
  // NOLINTNEXTLINE(cppcoreguidelines-init-variables)
  ...
  for (auto &segment : innerSegments) {
    ...
    for (auto i : thread_scope_stack_) {
      if (!exprEquals(current_thread_reach_[i], thread_extents[i])) {
        ...
        inner = alloc<Cond>(alloc<CompareSelect>(gpu_thread_vars_[i],
    ...
    for (auto i : block_scope_stack_) {
      if (!exprEquals(current_block_reach_[i], block_extents[i])) {
        // Mask it against the current dimensions.
        inner = alloc<Cond>(alloc<CompareSelect>(gpu_block_vars_[i],
...
void CudaCodeGenTssa::Initialize() {
  // TODO: handle multiple kernels.
  // TODO: handle dynamic dimension.
  // TODO: call nvrtc.
  // TODO: merge HasRand with CudaAnalysisTssa.
  GenericIntrinsicsExpander intrinsics_expander;
  apply_mutator(&intrinsics_expander);

  HasRand has_rand_func(stmt());
  has_random_ = has_rand_func.has_rand();
  cuda_analysis_ = std::make_unique<CudaAnalysisTssa>();
  printer_ = std::make_unique<CudaPrinterTssa>(&oss_, cuda_analysis_.get(),
                                               has_random_);
  metavar_rewriter_ =
      std::make_unique<GPUMetaVarRewriterTssa>(cuda_analysis_.get());
  ...
  // std::cout << to_string(stmt_v) << std::endl;
  stmt_v = stmt_v->accept_mutator(metavar_rewriter_.get());
  // std::cout << to_string(stmt_v) << std::endl;
  AtomicAddFuser atomic_add_fuser(cuda_analysis_->thread_local_bufs(),
                                  *metavar_rewriter_.get());
...
void CudaCodeGenTssa::call_with_numel(void **args, int64_t numel) {
...
void CudaCodeGenTssa::call_raw(const std::vector<void *> &raw_args) {
  auto const &buffer_args = this->buffer_args();
  ...
  // https://stackoverflow.com/questions/34388712/cannot-understand-how-jcuda-culaunchkernel-work
  for (size_t i = 0; i < buffer_args.size(); i++) {
    ptr_to_args[i] = buffer_args[i].isVar() ? raw_args[i]
                                            : const_cast<void **>(&raw_args[i]);
  }
  ...
  AT_CUDA_DRIVER_CHECK(nvrtc().cuLaunchKernel(
      ...
      gpu_thread_extents_v[2], 0, stream, ptr_to_args.data(), nullptr));
...
void CudaCodeGenTssa::call(const std::vector<CallArg> &args) {
...
at::Tensor
CudaCodeGenTssa::empty_strided(c10::IntArrayRef size, c10::IntArrayRef stride,
                               c10::optional<c10::ScalarType> dtype_opt,
                               c10::optional<c10::Layout> layout_opt,
                               c10::optional<c10::Device> device_opt,
                               c10::optional<bool> pin_memory_opt) {
  c10::DeviceGuard device_guard(device_opt.value());
  return at::native::empty_strided_cuda(size, stride, dtype_opt, layout_opt,
...
void CudaCodeGenTssa::CompileToNVRTC(const std::string &code,
  ...
  AT_CUDA_NVRTC_CHECK(nvrtc().nvrtcCreateProgram(&program, code.c_str(),
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-init-variables)
  const std::vector<const char *> args = {"--std=c++14", compute.c_str(),
                                          "-default-device"};
#endif
...
CudaCodeGenTssa::~CudaCodeGenTssa() = default;

RegisterCodeGen<CudaCodeGenTssa> cuda_codegen_tssa_reg("cuda_codegen_tssa");
