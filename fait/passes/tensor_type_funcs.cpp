...
void initTensorTypeFuncs() {
  ...
  std::initializer_list<std::pair<OperatorSet, void (*)(INFER_PARAMS)>>
      specialShapeHandlerInit{
          {reduceOps, handleShapeReduce},
          {sortOp, handleShapeSort},
          {topkOp, handleShapeTopk},
...
static bool faitShapeInferOperatorRegistered = false;
void faitInitShapeInferOperator() {
  if (faitShapeInferOperatorRegistered) return;
  faitShapeInferOperatorRegistered = true;
  ...
  squeezeOp.insert({"immut::squeeze(Tensor self, int dim) -> Tensor"});
  unsqueezeOp.insert({"immut::unsqueeze(Tensor self, int dim) -> Tensor"});
  reshapeOps.insert({
