...
template <class T>
static CustomLoweringFunction getComputeFillConst(
    T val,
...
static Tensor computeTriu(CUSTOM_LOWERING_PARAMS) {
  return Compute("triu", outShape, [&](ParameterList& axes) {
    ...
    return IfThenElse::make(
        (col - row) >= diagonal, self.load(axes), ExprHandle(immLike(self, 0)));
  });
}

static ExprHandle loadBcast(
    const BufHandle& srcBuf,
    const ShapeVec& dstShape,
    ParameterList& axes) {
...
static CustomLoweringFunction getComputeBinaryBcast(
    BinOp&& op,
    const std::string& name) {
  return [=](CUSTOM_LOWERING_PARAMS) {
...
static Tensor computeSoftmax(CUSTOM_LOWERING_PARAMS) {
  ...
  std::vector<StmtPtr> stmts{
      dimMax.stmt(), expSubMax.stmt(), dimSum.stmt(), result.stmt()};
  ...
  return Compute("cat", outShape, [&](ParameterList& axes) {
    ...
    for (int64_t i = bufs.size() - 1; i >= 0; i--) {
      ...
      result = IfThenElse::make(
          ExprHandle(dimAxis) < indices[i + 1],
          bufs[i].load(bufLoadAxes),
          result);
    }
...
static Tensor computeStack(CUSTOM_LOWERING_PARAMS) {
  return Compute("stack", outShape, [&](ParameterList& axes) {
    ...
    for (int64_t i = bufs.size() - 1; i >= 0; i--) {
      result = IfThenElse::make(
          ExprHandle(dimAxis) == i, bufs[i].load(loadAxes), result);
    }
...
static bool faitNNCLoweringFuncRegistered = false;
void faitInitNNCLoweringFunc() {
  if (faitNNCLoweringFuncRegistered) return;
  faitNNCLoweringFuncRegistered = true;
  customLoweringFuncs.insert({
    ...
    {"immut::repeat(Tensor self, int[] size) -> Tensor", computeRepeat},
